...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if ([self.results[indexPath.row] isKindOfClass:[NSString class]]) {
  ...
  } else if ([self.results[indexPath.row]
                 isKindOfClass:[TTInstruction class]]) {
    TTInstruction *instruction = (TTInstruction *)self.results[indexPath.row];
    ManeuverCell *maneuverCell =
        [[ManeuverCell alloc] initWithStyle:UITableViewCellStyleDefault
                            reuseIdentifier:@"reusableManeuverCellID"];
    NSInteger distanceToManeuver = instruction.routeOffsetInMetersValue;
    maneuverCell.maneuverDistance.text =
        [FormatUtils formatDistanceWithMeters:distanceToManeuver];
    maneuverCell.maneuverDescription.text = instruction.message;
    maneuverCell.maneuverImageView.image =
        [ManeuverIconGenerator imageForInstruction:instruction];
    cell = maneuverCell;
  } else if ([self.results[indexPath.row]
                 isKindOfClass:[TTSearchResult class]]) {
    TTSearchResult *searchResult =
        (TTSearchResult *)self.results[indexPath.row];
    double distance = 0;
    if (LocationManager.shared.lastLocation != nil) {
      distance = [LocationManager.shared.lastLocation
          distanceFromLocation:[[CLLocation alloc] init:searchResult.position]];
    }
    NSString *distanceAsText = [FormatUtils formatDistanceWithMeters:distance];
    NSString *text =
        [NSString stringWithFormat:@"%@ %@ %@ %@", distanceAsText,
                                   searchResult.address.country,
                                   searchResult.address.freeformAddress,
                                   searchResult.address.countryCode];
    cell.textLabel.numberOfLines = 4;
    cell.textLabel.text = text;
  } else if ([self.results[indexPath.row]
                 isKindOfClass:[TTTrafficDetailBase class]]) {
    TrafficCell *trafficCell =
        [[TrafficCell alloc] initWithStyle:UITableViewCellStyleDefault
                           reuseIdentifier:@"reusableTrafficCellID"];
    if ([self.results[indexPath.row] isKindOfClass:[TTTrafficDetail class]]) {
      ...
      trafficCell.trafficImageView.image =
          [TTTrafficDetailImage createWithTraffic:detail].medium;
      trafficCell.trafficDescription.text =
          [NSString stringWithFormat:@"(%@) / (%@)", detail.from, detail.to];
      trafficCell.trafficDelay.text =
          detail.delay == -1
              ? @"---"
              : [FormatUtils formatTimeDelayWithSeconds:detail.delay];
      trafficCell.trafficLength.text =
          [FormatUtils formatDistanceWithMeters:detail.length];
    } else if ([self.results[indexPath.row]
                   isKindOfClass:[TTTrafficDetailCluster class]]) {
      TTTrafficDetailCluster *cluster =
          (TTTrafficDetailCluster *)self.results[indexPath.row];
      trafficCell.trafficImageView.image =
          [TTTrafficDetailImage createWithTraffic:cluster].medium;
      trafficCell.trafficDescription.text = @"Multiple incidents";
      trafficCell.trafficLength.text =
          [FormatUtils formatDistanceWithMeters:cluster.length];
    }
    cell = trafficCell;
  }
  return cell;
