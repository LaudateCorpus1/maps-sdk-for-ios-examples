...
@interface RoutingSupportingPointsViewController () <TTRouteResponseDelegate,
                                                     TTRouteDelegate>
...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc] initWithLabels:@[ @"0 m", @"10 km" ]
                                              selectedID:-1];
}
...
- (void)displayDeviation10kmRoute {
  TTRouteQuery *query =
      [[[[[[TTRouteQueryBuilder createWithDest:[TTCoordinate PORTUGAL_COIMBRA]
          ...
          withMaxAlternatives:1] withMinDeviationTime:0]
          withSupportingPoints:self.suporrtingPoints
                         count:5] withMinDeviationDistance:10000] build];
  [self.routePlanner planRouteWithQuery:query];
}
