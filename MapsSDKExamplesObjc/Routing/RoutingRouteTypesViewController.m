...
- (OptionsView *)getOptionsView {
    return [[OptionsViewSingleSelect alloc] initWithLabels:@[ @"Fastest", @"Shortest", @"Eco" ] selectedID:-1];
}
...
- (void)displayShortestRoute {
    TTRouteQuery *query = [[[TTRouteQueryBuilder createWithDest:[TTCoordinate AMSTERDAM] andOrig:[TTCoordinate ROTTERDAM]] withRouteType:TTOptionTypeRouteShortest] build];
    [self.routePlanner planRouteWithQuery:query];
}

- (void)displayEcoRoute {
    TTRouteQuery *query = [[[TTRouteQueryBuilder createWithDest:[TTCoordinate AMSTERDAM] andOrig:[TTCoordinate ROTTERDAM]] withRouteType:TTOptionTypeRouteEco] build];
    [self.routePlanner planRouteWithQuery:query];
}
