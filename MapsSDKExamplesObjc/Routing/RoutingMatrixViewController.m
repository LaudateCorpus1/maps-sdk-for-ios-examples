...
- (void)setupCenterOnWillHappen {
    [self.mapView centerOnCoordinate:TTCoordinate.AMSTERDAM_CENTER_LOCATION withZoom:12];
}

- (OptionsView *)getOptionsView {
    return [[OptionsViewSingleSelect alloc] initWithLabels:@[@"One to Many", @"Many to Many"] selectedID:-1];
}
...
- (void)displayOneToMany {
    TTMatrixRouteQuery* query = [[[[[[TTMatrixRouteQueryBuilder createWithOrigin:TTCoordinate.AMSTERDAM_CENTER_LOCATION
                                   ...
                                   addDestination:TTCoordinate.AMSTERDAM_RESTAURANT_WAGAMAMA] addDestination:TTCoordinate.AMSTERDAM_RESTAURANT_ENVY]
                                 build];
    [self.matrix matrixRouteWithQuery:query];
}

- (void)displayManyToMany {
    TTMatrixRouteQuery* query = [[[[TTMatrixRouteQueryBuilder createWithOrigin:TTCoordinate.PASSENGER_ONE
                                                               withDestination:TTCoordinate.TAXI_ONE]
                                   addOrigin:TTCoordinate.PASSENGER_TWO] addDestination:TTCoordinate.TAXI_TWO]
                                 build];
    [self.matrix matrixRouteWithQuery:query];
}
...
- (UIColor*)determineColorWith:(TTMatrixRoutingResultKey*)matrixRoutingResultKey withResponse:(TTMatrixRouteResponse*)response {
    ...
    for(TTMatrixRouteResult* result in response.results.allValues) {
        if([CLLocation locationsEqualsWithFirst:result.origin second:matrixRoutingResultKey.origin]) {
