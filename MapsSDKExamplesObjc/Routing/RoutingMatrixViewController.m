...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.matrix = [[TTMatrixRoute alloc] initWithKey:Key.Routing];
...
- (void)displayManyToMany {
    TTMatrixRouteQuery *query = [[[[TTMatrixRouteQueryBuilder createWithOrigin:TTCoordinate.PASSENGER_ONE withDestination:TTCoordinate.TAXI_ONE] addOrigin:TTCoordinate.PASSENGER_TWO] addDestination:TTCoordinate.TAXI_TWO] build];
    [self.matrix matrixRouteWithQuery:query];
}
