...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"Initial order", @"Best order", @"No waypoints" ]
          selectedID:-1];
}
...
- (void)viewDidLoad {
  ...
  for (int i = 0; i < 3; i++) {
    [self.mapView.annotationManager
        addAnnotation:[TTAnnotation
                          annotationWithCoordinate:self.waypoints[i]]];
  }
}
  ...
  TTRouteQuery *query =
      [[[[TTRouteQueryBuilder createWithDest:[TTCoordinate BERLIN]
          ...
          withWayPoints:self.waypoints
                  count:3] withComputeBestOrder:YES] build];
  [self.routePlanner planRouteWithQuery:query];
}
