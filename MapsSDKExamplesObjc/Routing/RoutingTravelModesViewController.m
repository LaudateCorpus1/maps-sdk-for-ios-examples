...
- (void)displayTruckRoute {
    self.routeStyle = TTMapRouteStyle.defaultActiveStyle;
    TTRouteQuery *query = [[[TTRouteQueryBuilder createWithDest:[TTCoordinate AMSTERDAM] andOrig:[TTCoordinate ROTTERDAM]] withTravelMode:TTOptionTravelModeTruck] build];
    [self.routePlanner planRouteWithQuery:query];
}
...
- (void)displayPedestrianRoute {
    ...
    self.routeStyle = [[[[[[TTMapRouteStyleBuilder new] withLineCapType:TTLineCapTypeRound] withWidth:0.3] withFillColor:[TTColor BlueLight]] withDashArray:dashArray] build];

    TTRouteQuery *query = [[[TTRouteQueryBuilder createWithDest:[TTCoordinate HAARLEMDC] andOrig:[TTCoordinate HAARLEMZW]] withTravelMode:TTOptionTravelModePedestrian] build];
    [self.routePlanner planRouteWithQuery:query];
}
...
- (void)route:(TTRoute *)route completedWithResult:(TTRouteResult *)result {
    ...
    TTMapRoute *mapRoute = [TTMapRoute routeWithCoordinatesData:result.routes.firstObject withRouteStyle:self.routeStyle imageStart:TTMapRoute.defaultImageDeparture imageEnd:TTMapRoute.defaultImageDestination];
