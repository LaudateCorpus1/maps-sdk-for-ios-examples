...
@interface RoutingAlternativesRouteViewController () <TTRouteResponseDelegate,
                                                      TTRouteDelegate>
...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc] initWithLabels:@[ @"1", @"3", @"5" ]
                                              selectedID:-1];
}
...
- (void)displayRouteWith1Alternative {
  TTRouteQuery *query = [[[TTRouteQueryBuilder
      createWithDest:[TTCoordinate AMSTERDAM]
             andOrig:[TTCoordinate ROTTERDAM]] withMaxAlternatives:1] build];
...
- (void)displayRouteWith3Alternative {
  TTRouteQuery *query = [[[TTRouteQueryBuilder
      createWithDest:[TTCoordinate AMSTERDAM]
             andOrig:[TTCoordinate ROTTERDAM]] withMaxAlternatives:3] build];
  [self.routePlanner planRouteWithQuery:query];
}
...
- (void)displayRouteWith5Alternative {
  TTRouteQuery *query = [[[TTRouteQueryBuilder
      createWithDest:[TTCoordinate AMSTERDAM]
             andOrig:[TTCoordinate ROTTERDAM]] withMaxAlternatives:5] build];
  [self.routePlanner planRouteWithQuery:query];
}
...
- (void)route:(TTRoute *)route completedWithResult:(TTRouteResult *)result {
  ...
  for (TTFullRoute *planedRoute in result.routes) {
    if (activeRoute == nil) {
      ...
      [self.etaView showWithSummary:planedRoute.summary
