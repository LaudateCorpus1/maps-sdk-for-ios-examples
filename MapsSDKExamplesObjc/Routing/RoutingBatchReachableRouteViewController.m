...
@interface RoutingBatchReachableRouteViewController () <
    TTBatchRouteVisistor, TTBatchRouteResponseDelegate, TTAnnotationDelegate>
...
- (void)viewDidLoad {
  ...
  [self.etaView updateWithText:@"Touch polyline to see the description..."
...
- (void)batch:(TTBatchRoute *)route
    completedWithResponse:(TTBatchRouteResponse *)response {
  [self.progress hide];
  [response visit:self];
...
- (void)batch:(TTBatchRoute *)route
    failedWithError:(TTResponseError *)responseError {
...
- (void)visitReachableRange:(TTReachableRangeResponse *)response {
  CLLocationCoordinate2D *coordinates = malloc(
      sizeof(CLLocationCoordinate2D) * response.result.boundriesCount + 1);
  ...
  TTPolyline *polyline = [TTPolyline
      polylineWithCoordinates:coordinates
                        count:response.result.boundriesCount + 1
                      opacity:1
                        width:3
                        color:[self determinePolylineColor:self.index]];
...
- (void)annotationManager:(id<TTAnnotationManager>)manager
  ...
  if (polyline.tag != nil && [polyline.tag isKindOfClass:[NSString class]]) {
    [self.etaView updateWithText:(NSString *)polyline.tag
                            icon:[UIImage imageNamed:@"info_small"]];
  }
}
