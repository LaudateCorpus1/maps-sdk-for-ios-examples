...
@interface RoutingAvoidVignettesAndAreasViewController () <TTRouteDelegate,
...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"No avoids", @"Avoid Vignettes", @"Avoid area" ]
          selectedID:-1];
}
- (void)viewDidLoad {
  [super viewDidLoad];
...
- (void)startRoutingNoAvoids {
  ...
  TTRouteQuery *query = [[[TTRouteQueryBuilder
      createWithDest:TTCoordinate.ROMANIA
             andOrig:TTCoordinate.CZECH_REPUBLIC] withTraffic:YES] build];
  __weak RoutingAvoidVignettesAndAreasViewController *weakSelf = self;
  [self.routePlannerBasic
      planRouteWithQuery:query
       completionHandler:^(TTRouteResult *_Nullable result,
         ...
         if (error != nil) {
           ...
           if (result != nil) {
             ...
             [weakSelf displayRoute:@"No avoids"
                       plannedRoute:plannedRoute
                         routeStyle:[TTMapRouteStyle defaultActiveStyle]];
...
- (void)startRoutingAvoidsVignettes {
  ...
  dispatch_group_enter(group);
  [self.routePlannerAvoid
      planRouteWithQuery:query2
       completionHandler:^(TTRouteResult *_Nullable result,
                           TTResponseError *_Nullable error) {
         if (error != nil) {
           [weakSelf handleError:error];
           ...
           if (result != nil) {
             TTFullRoute *plannedRoute = result.routes.firstObject;
             [weakSelf displayRoute:@"Avoids Vignettes"
                       plannedRoute:plannedRoute
                         routeStyle:[weakSelf setRouteStyle:[TTColor Pink]
                                               outlineColor:[TTColor Purple]]];
           }
         }
         dispatch_group_leave(group);
       }];
...
- (void)startRoutingAvoidArea {
  ...
  TTPolyline *polyline = [TTPolyline polylineWithCoordinates:coordinateArray
                                                       count:5
                                                     opacity:1
                                                       width:4
                                                       color:[TTColor Blue]];
       ...
       completionHandler:^(TTRouteResult *_Nullable result,
         ...
         if (error != nil) {
           ...
           if (result != nil) {
             TTFullRoute *plannedRoute = result.routes.firstObject;
             [weakSelf
                 displayRoute:@"Avoid area"
                 plannedRoute:plannedRoute
                   routeStyle:[weakSelf setRouteStyle:[TTColor BlueLight]
                                         outlineColor:[TTColor BlueLight]]];
           }
         }
         dispatch_group_leave(group);
       }];
       ...
       completionHandler:^(TTRouteResult *_Nullable result,
         ...
         if (error != nil) {
           ...
           if (result != nil) {
             TTFullRoute *plannedRoute = result.routes.firstObject;
             [weakSelf
                 displayRoute:@"Avoids Vignettes"
                 plannedRoute:plannedRoute
                   routeStyle:[weakSelf setRouteStyle:[TTColor GreenBright]
                                         outlineColor:[TTColor GreenBright]]];
           }
         }
         dispatch_group_leave(group);
       }];
...
- (void)displayRoute:(NSString *)etaView
        plannedRoute:(TTFullRoute *)plannedRout
          routeStyle:(TTMapRouteStyle *)routeStyle {
  TTMapRoute *mapRoute = [TTMapRoute
      routeWithCoordinatesData:plannedRout
  ...
  [self.etaView updateWithEta:etaView
               metersDistance:[plannedRout.summary lengthInMetersValue]];
  [self displayRouteOverview];
}

#pragma mark Drow route
- (TTMapRouteStyle *)setRouteStyle:(UIColor *)fillColor
                      outlineColor:(UIColor *)outlineColor {
  TTMapRouteStyle *routeStyle =
      [[[[[[TTMapRouteStyleBuilder alloc] init] withWidth:1.0]
          withFillColor:fillColor] withOutlineColor:outlineColor] build];
  return routeStyle;
}
