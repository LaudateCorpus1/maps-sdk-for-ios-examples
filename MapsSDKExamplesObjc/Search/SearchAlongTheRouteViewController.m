...
- (void)searchForEVStations {
    TTAlongRouteSearchQuery *query = [[[TTAlongRouteSearchQueryBuilder withTerm:@"ELECTRIC_VEHICLE_STATION" withRoute:self.mapRoute withMaxDetourTime:900] withLimit:10] build];
    [self.alongRouteSearch searchWithQuery:query];
}
...
- (void)route:(TTRoute *)route completedWithResult:(TTRouteResult *)result {
    ...
    self.mapRoute = [TTMapRoute routeWithCoordinatesData:result.routes.firstObject withRouteStyle:TTMapRouteStyle.defaultActiveStyle imageStart:TTMapRoute.defaultImageDeparture imageEnd:TTMapRoute.defaultImageDestination];
