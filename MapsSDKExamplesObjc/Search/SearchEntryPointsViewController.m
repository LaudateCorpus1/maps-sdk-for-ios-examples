...
- (void)displayEntryPointsForAirport {
    ...
    TTSearchQuery *query = [[TTSearchQueryBuilder createWithTerm:@"Amsterdam Airport Schiphol"] build];
    [self.search searchWithQuery:query];
}

- (void)displayEntryPointsForShoppingMall {
    self.text = @"Kalvertoren";
    TTSearchQuery *query = [[[TTSearchQueryBuilder createWithTerm:@"Kalvertoren"] withIdxSet:TTSearchIndexPointOfInterest] build];
    [self.search searchWithQuery:query];
}
