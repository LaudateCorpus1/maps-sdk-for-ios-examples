...
- (OptionsView *)getOptionsView {
    return [[OptionsViewSingleSelect alloc] initWithLabels:@[@"Country", @"Municipality"] selectedID:0];
}
...
- (void)viewDidLoad {
    ...
    [self setupEtaView];
    [self.etaView updateWithText:@"Drop a pin to get a polygon" icon:[UIImage imageNamed:@"info_small"]];
}
...
- (void)resolveAddressForCoordinates:(CLLocationCoordinate2D)coordinate {
    TTReverseGeocoderQuery *query = [[[TTReverseGeocoderQueryBuilder createWithCLLocationCoordinate2D:coordinate]
                                      withEntityType:self.entityType]
                                     build];
