...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"Bar", @"Gas", @"Parking" ]
          selectedID:-1];
}
...
- (void)batch:(TTBatchSearch *)batch
    completedWithResponse:(TTBatchResponse *)response {
  [self.progress hide];
  [response visit:self];
}
...
- (void)visitGeometrySearch:(TTGeometrySearchResponse *)response {
  ...
  TTCircle *circle =
      [TTCircle circleWithCenterCoordinate:[TTCoordinate HOOFDDORP]
                                    radius:4000
                                     width:2
