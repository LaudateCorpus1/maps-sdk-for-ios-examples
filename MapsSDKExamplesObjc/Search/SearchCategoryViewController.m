...
- (void)searchCategoryGas {
    TTSearchQuery *query = [[[[TTSearchQueryBuilder createWithTerm:@"gas"] withCategory:YES] withPosition:self.locationManager.lastLocation.coordinate] build];
    [self.search searchWithQuery:query];
}

- (void)searchCategoryATM {
    TTSearchQuery *query = [[[[TTSearchQueryBuilder createWithTerm:@"atm"] withCategory:YES] withPosition:self.locationManager.lastLocation.coordinate] build];
    [self.search searchWithQuery:query];
}
