...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"Parking", @"ATM", @"Grocery" ]
          selectedID:-1];
}
...
- (void)createShapesForSearch {
  ...
  [self.geometryShape
      addObject:[TTSearchPolygon polygonWithCoordinates:coordinates count:13]];

  TTCircle *mapCircle =
      [TTCircle circleWithCenterCoordinate:[TTCoordinate AMSTERDAM_CIRCLE]
                                    radius:2000
                                     width:2
                                     color:[TTColor RedSemiTransparent]
                                      fill:YES
                               colorOutlet:[TTColor RedSemiTransparent]];
  [self.mapView.annotationManager addOverlay:mapCircle];
  [self.geometryShape
      addObject:[TTSearchCircle circleWithCenter:[TTCoordinate AMSTERDAM_CIRCLE]
                                          radius:2000]];
}
...
- (void)search:(TTGeometrySearch *)search
    completedWithResponse:(TTGeometrySearchResponse *)response {
  [self.progress hide];
