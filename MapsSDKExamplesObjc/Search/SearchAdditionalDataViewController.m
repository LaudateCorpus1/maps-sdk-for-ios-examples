...
@interface SearchAdditionalDataViewController () <
    TTSearchDelegate, TTAdditionalDataSearchDelegate>
...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"Poland", @"Amsterdam", @"Schiphol" ]
          selectedID:-1];
}
...
- (void)displayAdditionalDataAmsterdam {
  TTSearchQuery *query = [[[TTSearchQueryBuilder createWithTerm:@"Amsterdam"]
      withIdxSet:TTSearchIndexGeographies] build];
  [self.search searchWithQuery:query];
}

- (void)displayAdditionalDataLanduse {
  TTSearchQuery *query =
      [[[TTSearchQueryBuilder createWithTerm:@"Amsterdam Airport Schiphol"]
