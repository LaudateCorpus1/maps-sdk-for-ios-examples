...
- (void)viewDidLoad {
  ...
  [self.etaView updateWithText:@"Drag a pin inside/outside of a fence"
                          icon:[UIImage imageNamed:@"info_small"]];
}

- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"One Fence", @"Two Fences" ]
          selectedID:-1];
}
...
- (void)displayExampleWithID:(NSInteger)ID on:(BOOL)on {
  ...
  switch (ID) {
  case 1:
    [self drawAmsterdamCircle:[TTCoordinate AMSTERDAM_CIRCLE_CENTER]
                   withRadius:[TTLocationDistance
                                  AMSTERDAM_CIRCLE_CENTER_RADIUS]];
  ...
  [self requestGeofencingReport:_draggableAnnotation.coordinate
                  withProjectId:_projectIdActive];
}

- (void)requestGeofencingReport:(CLLocationCoordinate2D)coorinate
...
- (void)responseGeofencing:(TTGeofencingReport *)report {
  ...
  [report.inside
      enumerateObjectsUsingBlock:^(
          TTGeofencingReportFenceDetails *_Nonnull reportFenceDetails,
          NSUInteger idx, BOOL *_Nonnull stop) {
        [self addMarker:reportFenceDetails.closestPoint.coordinate
              withTitle:reportFenceDetails.entity.name];
...
- (void)addMarker:(CLLocationCoordinate2D)coordinate
  ...
  FenceAnnotation *annotation = [[FenceAnnotation alloc]
...
- (void)draggableAnnotationPin {
  _draggableAnnotation =
      [[TTAnnotation alloc] initWithCoordinate:TTCoordinate.AMSTERDAM];
...
- (void)annotationManager:(id<TTAnnotationManager>)manager
       draggingAnnotation:(TTAnnotation *)annotation
                stateDrag:(TTAnnotationDragState)state {
  [manager deselectAnnotation];
  if (state == TTAnnotationViewDragStateIdle) {
    [self requestGeofencingReport:annotation.coordinate
                    withProjectId:_projectIdActive];
  }
}
...
- (UIView<TTCalloutView> *)annotationManager:(id<TTAnnotationManager>)manager
  ...
  if ([selectedAnnotation isKindOfClass:[FenceAnnotation class]]) {
    ...
    return [[TTCalloutOutlineView alloc]
        initWithUIView:
            [self labelForText:[NSString stringWithFormat:
  ...
  return [[TTCalloutOutlineView alloc]
      initWithUIView:
          [self labelForText:[self createDescriptionWithInside:_inside
                                                   withOutside:_outside]]];
}

- (void)removeAnnotationFences {
  NSMutableArray *annotationsToRemove = [NSMutableArray
      arrayWithArray:self.mapView.annotationManager.annotations];
