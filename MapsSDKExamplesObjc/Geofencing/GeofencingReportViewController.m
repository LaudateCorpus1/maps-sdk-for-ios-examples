...
- (UIView<TTCalloutView> *)annotationManager:(id<TTAnnotationManager>)manager viewForSelectedAnnotation:(TTAnnotation *)selectedAnnotation {
    if ([selectedAnnotation isKindOfClass:[FenceAnnotation class]]) {
        ...
        return [[TTCalloutOutlineView alloc] initWithUIView:
                [self labelForText:[NSString stringWithFormat:@" This is the closest location to \n   \"%@\" border", title]]];
    }
    return [[TTCalloutOutlineView alloc] initWithUIView:
            [self labelForText:[self createDescriptionWithInside:_inside withOutside:_outside]]];
}
