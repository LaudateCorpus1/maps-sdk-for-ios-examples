...
@interface MapDynamicMapSourcesViewController ()
...
- (void)setupCenterOnWillHappen {
    [self.mapView centerOnCoordinate:[TTCoordinate BUCKINGHAM_PALACE_CENETER] withZoom:16];
}

- (OptionsView *)getOptionsView {
    return [[OptionsViewMultiSelect alloc] initWithLabels:@[@"Image source", @"GeoJson source"] selectedID:-1];
}

- (void)addGeoJSONSource {
    NSString* path = [[NSBundle mainBundle] pathForResource:@"geojson_source" ofType:@"json"];
...
- (void)addImageSource {
    ...
    TTLatLngQuad* quad = [[TTLatLngQuad alloc] initWithTopLeft:TTCoordinate.BUCKINGHAM_PALACE_TOP_LETF
...
- (void)displayExampleWithID:(NSInteger)ID on:(BOOL)on {
    ...
    switch (ID) {
        case 1:
            geoLayer.visibility = on ? TTMapLayerVisibilityVisible : TTMapLayerVisibilityNone;
            break;
        default:
            imgLayer.visibility = on ? TTMapLayerVisibilityVisible : TTMapLayerVisibilityNone;
            break;
    }
}
