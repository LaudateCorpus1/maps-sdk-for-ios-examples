...
- (OptionsView *)getOptionsView {
  return [[OptionsViewSingleSelect alloc]
      initWithLabels:@[ @"Images", @"GeoJSON", @"Roads" ]
          selectedID:0];
}
...
- (void)onMapReady {
  ...
  NSString *path = [NSBundle.mainBundle pathForResource:@"layer_road"
                                                 ofType:@"json"];
  NSString *layerJSON =
...
- (void)planRoute {
  TTRouteQuery *query =
      [[TTRouteQueryBuilder createWithDest:TTCoordinate.SANTA_CRUZ
                                   andOrig:TTCoordinate.SAN_FRANCISCO] build];
  [self.routePlanner planRouteWithQuery:query];
}

- (void)addImagesSource {
  [self addImage:1
      withCoordinates:TTCoordinate.SAN_JOSE_IMG1
            withImage:[UIImage imageNamed:@"img1"]];
  [self addImage:2
      withCoordinates:TTCoordinate.SAN_JOSE_IMG2
            withImage:[UIImage imageNamed:@"img2"]];
  [self addImage:3
      withCoordinates:TTCoordinate.SAN_JOSE_IMG3
            withImage:[UIImage imageNamed:@"img3"]];
}
...
- (void)addImage:(int)index
  ...
  NSMutableString *imageID =
      [NSMutableString stringWithFormat:@"img-source-%d", index];
  TTMapImageSource *sourceMap = [TTMapImageSource createWithID:imageID
                                                         image:uiImage
                                                   coordinates:quad];
  [self.currentStyle addSource:sourceMap];

  NSMutableString *rasterID =
      [NSMutableString stringWithFormat:@"img-id-%d", index];
  NSMutableString *sourceID =
      [NSMutableString stringWithFormat:@"img-source-%d", index];
  TTMapLayer *layerMap = [TTMapLayer createRasterWithID:rasterID
...
- (void)route:(TTRoute *)route completedWithResult:(TTRouteResult *)result {
  ...
  TTGeoJSONLineString *geoJsonLine = [[TTGeoJSONLineString alloc]
      initWithCoordinatesData:[fullRoute coordinatesData]
              withBoundingBox:nil];
  TTMapGeoJSONSource *geoJsonSource =
      [TTMapGeoJSONSource createWithID:GEOJSON_SOURCE];
...
- (void)displayExampleWithID:(NSInteger)ID on:(BOOL)on {
  ...
  switch (ID) {
  case 2: {
    NSArray<TTMapLayer *> *layersRoads = [self.currentStyle
        getLayersBySourceLayerRegexs:[NSArray
                                         arrayWithObjects:@".*[rR]oad.*",
                                                          @".*[mM]otorway.*",
                                                          nil]];
  ...
  default: {
    NSArray<TTMapLayer *> *layersImages =
        [self.currentStyle getLayersByRegex:@"img-id.*"];
...
- (TTLatLngQuad *)quadWithDelta:(CLLocationCoordinate2D)coordinate
                      withDelta:(double)delta {

  return [[TTLatLngQuad alloc]
      initWithTopLeft:CLLocationCoordinate2DMake(coordinate.latitude +
                                                     delta / 2,
                                                 coordinate.longitude - delta)
         withTopRight:CLLocationCoordinate2DMake(coordinate.latitude +
                                                     delta / 2,
                                                 coordinate.longitude + delta)
      withBottomRight:CLLocationCoordinate2DMake(coordinate.latitude -
                                                     delta / 2,
                                                 coordinate.longitude - delta)
       withBottomLeft:CLLocationCoordinate2DMake(coordinate.latitude -
                                                     delta / 2,
                                                 coordinate.longitude + delta)];
}
