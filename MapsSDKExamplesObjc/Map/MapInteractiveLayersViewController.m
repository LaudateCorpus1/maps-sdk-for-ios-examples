...
@interface MapInteractiveLayersViewController () <TTMapViewDelegate, TTMapViewCameraDelegate>
...
- (void)onMapReady {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.toast.delayTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      [self displayFeaturesToast];
    });
}

- (void)addGeoJSONSource {
    NSString *path = [[NSBundle mainBundle] pathForResource:@"interactive_layers_data" ofType:@"json"];
    ...
    NSString *outlinePath = [[NSBundle mainBundle] pathForResource:@"interactive_layers_outline_layer" ofType:@"json"];
    NSString *outlineLayerJson = [NSString stringWithContentsOfFile:outlinePath encoding:NSUTF8StringEncoding error:nil];
    TTMapLayer *outlineLayer = [TTMapLayer createWithStyleJSON:outlineLayerJson withMap:self.mapView];
    [self.currentStyle addLayer:outlineLayer];

    NSString *fillPath = [[NSBundle mainBundle] pathForResource:@"interactive_layers_fill_layer" ofType:@"json"];
    NSString *fillLayerJson = [NSString stringWithContentsOfFile:fillPath encoding:NSUTF8StringEncoding error:nil];
    TTMapLayer *fillLayer = [TTMapLayer createWithStyleJSON:fillLayerJson withMap:self.mapView];
    [self.currentStyle addLayer:fillLayer];
}

- (NSString *)listFeaturesIdsToString:(TTGeoJSONFeatureCollection *_Nonnull)geoJSONFeatureCollection {
...
- (void)displayFeaturesToast {
    ...
    NSMutableString *message = [NSMutableString stringWithString:@"Features in viewport: "];
