...
- (void)setupInitialCameraPosition {
    [self.mapView centerOnCoordinate:[TTCoordinate AMSTERDAM_CENTER_LOCATION] withZoom:9];
}
...
- (void)setupClusterStyle {
    ...
    [self addLayerWithName:@"ic_app_test_layer" toMap:self.mapView andStyle:self.currentStyle];
    [self addLayerWithName:@"ic_app_test_layer_cluster" toMap:self.mapView andStyle:self.currentStyle];
    [self addLayerWithName:@"ic_app_test_layer_symbol_count" toMap:self.mapView andStyle:self.currentStyle];
}

- (void)addLayerWithName:(NSString *_Nonnull)jsonStyleName toMap:(TTMapView *_Nonnull)map andStyle:(TTMapStyle *_Nonnull)style {
